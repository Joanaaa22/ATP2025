...
def verifica_obito(dados_doente):
    """
    Verifica se um doente falece após a consulta.
    Apenas pulseira vermelha tem risco.
    """
...
def simula_eventos(
    ...
    # Nota: Usamos seed apenas para consistência nos gráficos de comparação (Relatórios)
    # Se quiseres resultados aleatórios sempre, comenta estas linhas.
    # random.seed(42)
    # np.random.seed(42)
        ...
        if tipo == CHEGADA:
            ...
            if pessoas_db and d_id in pessoas_db:
                ...
                if p_bd:
                    ...
                    # --- REGISTRA O TEMPO DE ATENDIMENTO ---
...
def ordenar_por_atendimento(bd):
    ...
    for d in bd:
        if not isinstance(d, dict): continue
        # Verifica se tem timestamp de atendimento e se não é None
        if "timestamp_atendimento" in d and d["timestamp_atendimento"] is not None:
            atendidos.append(d)
        else:
            outros.append(d)
            
    # Ordena os atendidos pelo tempo (crescente)
    atendidos.sort(key=lambda x: x["timestamp_atendimento"])
    
    # Ordena os outros por ID (para manter consistência visual)
...
def grafico_barras_atendidos_falecidos(bd):
    ...
    # Conta os falecidos (estado = FALECIDO)
    falecidos = sum(1 for d in bd if isinstance(d, dict) and str(d.get("estado", "")).upper() == "FALECIDO")
    
    # Conta os atendidos vivos:
    # 1. Tem timestamp de atendimento (passou pela simulação)
    # 2. Estado não é FALECIDO (está ATIVO ou INATIVO)
...
def grafico_circular_pulseiras_por_especialidade(bd):
    ...
    for d in bd:
        if not isinstance(d, dict): continue
        # Apenas conta doentes atendidos nesta simulação (com timestamp)
        if not ("timestamp_atendimento" in d and d["timestamp_atendimento"] is not None):
            continue
            
        estado = str(d.get("estado", "")).upper()
        if estado == "FALECIDO": continue # Opcional: contar ou não falecidos na especialidade. Vamos assumir que sim, pois foram atendidos.
...
while True:
    ...
    if event == "-RUN_SIM-":
        ...
        try:
            ...
            # Limpar timestamps antigos antes de nova simulação principal?
            # Para efeitos de relatório, mantemos a estrutura. 
            # A simulação vai sobrescrever timestamps se o mesmo doente for atendido de novo.
            ...
            medias = []
            for tx in taxas:
                # CORREÇÃO CRÍTICA: Usar deepcopy para não poluir a BD real com os testes
            ...
            # ATUALIZAÇÃO AUTOMÁTICA DOS GRÁFICOS DE BD
